...
def _mock_edgeface_model(embedding_size=512):
    """Create a simple mock model for testing without downloading."""
    class MockEdgeFace(torch.nn.Module):
        def __init__(self, embedding_size):
            super().__init__()
            self.conv1 = torch.nn.Conv2d(3, 64, kernel_size=3, padding=1)
            self.conv2 = torch.nn.Conv2d(64, 128, kernel_size=3, padding=1)
            self.pool = torch.nn.AdaptiveAvgPool2d(4)
            self.fc = torch.nn.Linear(128 * 4 * 4, embedding_size)
    ...
    return MockEdgeFace(embedding_size)

# Define all EdgeFace variants
def base(pretrained=False):
    """EdgeFace base model"""
    model = _mock_edgeface_model(512)
    print("Loaded mock EdgeFace base model")
    return model

def s_gamma_05(pretrained=False):
    """EdgeFace small model with gamma 0.5"""
    model = _mock_edgeface_model(512)
    print("Loaded mock EdgeFace s_gamma_05 model")
    return model

def xs_q(pretrained=False):
    """EdgeFace extra small quantized model"""
    model = _mock_edgeface_model(256)
    print("Loaded mock EdgeFace xs_q model (quantized)")
    return model

def xs_gamma_06(pretrained=False):
    """EdgeFace extra small model with gamma 0.6"""
    model = _mock_edgeface_model(256)
    print("Loaded mock EdgeFace xs_gamma_06 model")
    return model

def xxs(pretrained=False):
    """EdgeFace extra-extra small model"""
    model = _mock_edgeface_model(128)
    print("Loaded mock EdgeFace xxs model")
    return model

def xxs_q(pretrained=False):
    """EdgeFace extra-extra small quantized model"""
    model = _mock_edgeface_model(128)
    print("Loaded mock EdgeFace xxs_q model (quantized)")
    return model
